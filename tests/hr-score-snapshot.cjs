...
const current={...row,lineup:{status:'confirmed',spot:1},contactQuality:{...row.contactQuality,checkedAt:new Date().toISOString()}};
const cases=score([current,{...current,research:{...current.research,profile:{windows:[{window:20,n:20,hrGames:0}]}}},{...current,lineup:{status:'pending'}},{...current,contactQuality:{...current.contactQuality,checkedAt:'2020-01-01T00:00:00Z'}}]);
assert.equal(cases[0].experiments['no-streak-v1'].score,cases[1].experiments['no-streak-v1'].score);
assert.notEqual(cases[0].score,cases[1].score);
assert.equal(cases[2].experiments['no-streak-v1'].score,null);
assert.equal(cases[3].experiments['contact-first-v1'].score,null);
assert.ok(Number.isFinite(cases[0].experiments['contact-first-v1'].score));
console.log('PASS challengers remove streak contribution, require confirmed lineup, and reject stale contact');
